...
class TelegramAdapter:
    ...
    def __init__(
        ...
        events: EventRouter,
        *,
        client_factory: Callable[[Settings], TelegramClient] | None = None,
    ) -> None:
    ...
    def _validate_credentials(self) -> None:
        if not self.settings.telegram_api_id or not self.settings.telegram_api_hash:
            raise ConfigurationError(
                "TELEGRAM_API_ID and TELEGRAM_API_HASH are required for Telegram startup"
            )
        try:
            int(self.settings.telegram_api_id)
        except ValueError as exc:
            raise ValidationError("TELEGRAM_API_ID must be an integer") from exc

    def _build_client(self) -> TelegramClient:
        if self._client_factory is not None:
            return self._client_factory(self.settings)

        try:
            from telethon import TelegramClient as TelethonClient
        except ImportError as exc:
            raise DependencyError(
                "Telethon is not installed; Telegram integration is unavailable",
                retryable=False,
            ) from exc

        session = self.settings.telegram_session or "selfbot"
    ...
    async def stop(self) -> None:
        if not self._started and self._client is None:
            return
    ...
    async def send_message(self, chat_id: str | int, text: str) -> Any:
        if not self._started or self._client is None:
            raise DependencyError("Telegram client is not started", retryable=False)
        if not text.strip():
    ...
    async def _on_new_message(self, event: Any) -> None:
        ...
        actor = getattr(message, "sender_id", None)
        text_value = getattr(message, "message", None)
        ...
        await self.events.dispatch(envelope)
